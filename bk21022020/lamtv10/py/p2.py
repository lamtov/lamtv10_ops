...
instances_uuid=connection.execute(db.select([nova_instances.columns.uuid]).where(db.and_(nova_instances.columns.display_name==instance_name , nova_instances.columns.vm_state=="active" ))).fetchall()[0][0]
...
instance_info=connection.execute(db.select([nova_instance_extra.columns.numa_topology]).where(nova_instances.columns.uuid==instances_uuid)).fetchall()[0][0]
