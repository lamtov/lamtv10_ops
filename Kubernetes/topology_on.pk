...
def isolcpus():
    ...
    cpus = []
    system_config = open("/etc/systemd/system.conf", "r")

    for line in system_config:

        if re.match("(^CPUAffinity.*$)", line):
            #print( line)
            pair = line.split("=")
            if len(pair) !=2:
                continue 
            ...
            not_cpus = parse_cpus_from_isolcpus(cpus_str)
            for cp in range(0,total_cpus):
                if cp not in not_cpus:
                    cpus.append(cp)
