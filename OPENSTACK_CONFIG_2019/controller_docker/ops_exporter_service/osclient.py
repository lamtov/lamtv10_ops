...
class OSClient(object):
    ...
    #lamtv10 edit here 25/09/2019
    def manual_edit_catalog(self, catalog):
        for i  in range(0,len(catalog)):
            if catalog[i]['name']=='placement' : 
                catalog[i]['url']=os.getenv('PLACEMENT_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('PLACEMENT_ADMIN_URL')

            if catalog[i]['name']=='neutron' : 
                catalog[i]['url']=os.getenv('NEUTRON_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('NEUTRON_ADMIN_URL')

            if catalog[i]['name']=='keystone' : 
                catalog[i]['url']=os.getenv('KEYSTONE_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('KEYSTONE_ADMIN_URL')

            if catalog[i]['name']=='nova' : 
                catalog[i]['url']=os.getenv('NOVA_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('NOVA_ADMIN_URL')

            if catalog[i]['name']=='cinderv3' : 
                catalog[i]['url']=os.getenv('CINDERV3_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('CINDERV3_ADMIN_URL')

            if catalog[i]['name']=='cinderv2' : 
                catalog[i]['url']=os.getenv('CINDERV2_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('CINDERV2_ADMIN_URL')

            if catalog[i]['name']=='swift' : 
                catalog[i]['url']=os.getenv('SWIFT_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('SWIFT_ADMIN_URL')
                
            if catalog[i]['name']=='glance' : 
                catalog[i]['url']=os.getenv('GLANCE_INTERNAL_URL')
                catalog[i]['admin_url']=os.getenv('GLANCE_ADMIN_URL')
...
#####################################################################################
    ...
    def get_token(self):
        ...
        for item in data['token']['catalog']:
            ...
            self.manual_edit_catalog(self._service_catalog)
